...
public class Logo extends Element {
    ...
    public Border render() {
        ...
        int sideLength = 28;
        String clientName = Client.getName().toUpperCase();
        ...
        RenderUtils.drawRoundRect(0,0,borderWidth,borderHeight,8,ColorUtils.transparent());
        RenderUtils.drawGradientRect(0,0,borderWidth,intervalY,Palette.getColor1().getRGB(),Palette.getColor2().getRGB());
        ...
        startX += sideLength + intervalX;
        {
            ...
            for (char c : clientName.toCharArray()) {
                float y = startY + sideLength / 2F - font.getStringHeight(clientName) / 2F + intervalY / 2F;
